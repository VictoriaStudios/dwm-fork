...
void cleanup(void) {
  ...
  for (i = 0; i < LENGTH(colors); i++) drw_scm_free(drw, scheme[i], 3);
  free(scheme);
...
Atom getatomprop(Client* c, Atom prop) {
  ...
  if (XGetWindowProperty(dpy, c->win, prop, 0L, sizeof atom, False, req, &da,
      ...
      p) {
    atom = *(Atom*)p;
    if (da == xatom[XembedInfo] && nitems == 2) atom = ((Atom*)p)[1];
    XFree(p);
  }
  return atom;
}
...
long getstate(Window w) {
  ...
  if (XGetWindowProperty(dpy, w, wmatom[WMState], 0L, 2L, False,
                         ...
                         &p) != Success)
    return -1;
  if (n != 0 && format == 32) result = *(long*)p;
  XFree(p);
  return result;
}
  ...
  do {
    ...
    switch (ev.type) {
      ...
      case MapRequest:
        ...
        break;
      case MotionNotify:
        if ((ev.xmotion.time - lasttime) <= (1000 / refreshrate)) continue;
...
void tile(Monitor* m) {
  ...
  for (i = my = ty = 0, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      if (my + HEIGHT(c) < m->wh) my += HEIGHT(c) + gappx;
    } else {
      r = n - i;
